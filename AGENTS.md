...
Shared instructions for every AI coding agent on this repo: Cursor, Claude Code, Codex and Antigravity. Claude Code loads this file through `CLAUDE.md`. Keep it under 12,000 characters (Antigravity's per-file limit).
...
- Describe what a feature does from the code, not from README or docs, which have been wrong before.

## Multiple agents, one repo

- Several agents may be live in the same clone. Before switching branches, run `git status`; if there is uncommitted work you didn't make, use a separate `git worktree` instead. Stage explicit paths only.
- Create branches from the remote base: `git checkout -b <name> origin/main`.

## Shared memory — where lessons go

Every agent reads the repo; no agent reads another tool's private memory. When you learn something worth keeping, write it into the committed file below in the same change, not only into Cursor rules in your home folder, Claude memory or chat.

| Kind of lesson | Where |
|---|---|
| Durable fact or rule about this code (architecture, gotchas, conventions) | This file (keep it short) or a linked doc in `docs/` |
| Coding rules Cursor enforces | `RULE.md` and `.cursor/rules/*.mdc` — update this file's summary if they change |
| Open threads: work handed to another chat, pending decisions, unverified fixes | [docs/agents/OPEN-THREADS.md](docs/agents/OPEN-THREADS.md), dated; remove entries when resolved |
| Known bugs | [known_bugs.md](known_bugs.md) |

Before starting work that sounds like an open thread, check `OPEN-THREADS.md`, `git log` and `gh pr list --state all` to see whether it's already done.
